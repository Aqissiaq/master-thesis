...
for all objects A, B and C~\footnote{For more on adjunctions in general, and this adjunction in particular see
IV.6: Cartesian Closed Categories in~\cite{maclane1998}} which means we can write the type of a function which
takes multiple arguments as a sequence of types with right arrows (associating to the right).
